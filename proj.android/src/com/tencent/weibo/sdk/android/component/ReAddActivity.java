...
public class ReAddActivity extends Activity {
	...
	public View initLayout(){
		...
		layout.setBackgroundDrawable(BackGroudSeletor.getdrawble("tencentWeibo/readd_bg", getApplication()));
		RelativeLayout cannelLayout = new RelativeLayout(this);
		cannelLayout.setLayoutParams(fillWrapParams);
		cannelLayout.setBackgroundDrawable(BackGroudSeletor.getdrawble("tencentWeibo/up_bg2x", getApplication()));
		cannelLayout.setGravity(LinearLayout.HORIZONTAL);
		
		Button returnBtn = new Button(this);
		String[] pngArray = new String[]{"tencentWeibo/quxiao_btn2x","tencentWeibo/quxiao_btn_hover"};
	...
	//	contentLayout.setBackgroundColor(Color.GRAY);//修改此处 设置背景
		...
		contentLayout.setBackgroundDrawable(BackGroudSeletor.getdrawble("tencentWeibo/input_bg", getApplication()));
		...
		gallery.setBackgroundDrawable(BackGroudSeletor.getdrawble("tencentWeibo/pic_biankuang2x", getApplication()));
		requestForGallery();
   		...
   		public void onResult(Object object) {
   			...
   			if(result.isExpires()){
   				...
   				if(result.isSuccess()){
   	   				...
   	   				IAPJni.shareSuccess(2);
   	   				ReAddActivity.this.finish();
   	   			}else{
   	   				Toast.makeText(ReAddActivity.this, result.getError_message(), Toast.LENGTH_SHORT).show();
   	   				IAPJni.shareFailed(2);
	   				ReAddActivity.this.finish();
   	   			}
   			}
   		}
