...
void Detect::setPpJoy(PpJoyPanel* ppJoy)
{
	_ppJoy = ppJoy;
	// 设置回调接口
	_ppJoy->onHit = CC_CALLBACK_2(Detect::onHit,this);
	_ppJoy->onJump = CC_CALLBACK_2(Detect::onJump,this);
	_ppJoy->onDirection = CC_CALLBACK_1(Detect::onDirection,this);
	_ppJoy->setEnabled(true);
}

PpJoyPanel* Detect::getPpJoy()
{
	return _ppJoy;
}
...
void Detect::resetCurTrial()
{
	_curTrial = nullptr;
}

Trial* Detect::getCurTrial()
{
	return _curTrial;
}

void Detect::setCurTrial(int trialId)
{
	// 当前关卡返回
	if(_curTrial != nullptr && trialId == _curTrial->getId())
		return;
	_curTrial = getWorld()->getTrial(trialId);
}
...
void Detect::onKeyPressed(EventKeyboard::KeyCode keyCode, Event* event)
{
	...
	{
		onJump(this, (int)cocos2d::ui::Widget::TouchEventType::BEGAN);
	}
	else if(keyCode == EventKeyboard::KeyCode::KEY_CTRL)
	{
		onHit(this, (int)cocos2d::ui::Widget::TouchEventType::BEGAN);
	}
	if(guide != Actor::Direction::STOP)
	{
		if(_player->getGrabCell() != nullptr)
		{
			_player->grabCell(_player->getGrabCell(), guide);
		}
		else if(_player)
		{
			_player->setGuide(guide,1);
		}
	}
}
...
void Detect::onKeyReleased(EventKeyboard::KeyCode keyCode, Event* event)
{
	...
	{
		onJump(this, (int)cocos2d::ui::Widget::TouchEventType::ENDED);
	}
	else if(keyCode == EventKeyboard::KeyCode::KEY_CTRL)
	{
		onHit(this, (int)cocos2d::ui::Widget::TouchEventType::ENDED);
	}
	if(guide != Actor::Direction::STOP)
	{
		if(_player)
		{
			_player->setGuide(guide,0);
		}		
	}
}

void Detect::onDirection(int enums)
{
	if(_controlled == false || _player == nullptr) return;

	// 重置导向
	_player->resetGuides();

	bool undo = false;
	Cell::Direction guide;
	switch (enums)
	{
	case (int)JoyStickPanel::DEFAULT:
		undo = true;
		break;
	case (int)JoyStickPanel::D_UP:
		guide = Cell::Direction::UP;
		break;
	case (int)JoyStickPanel::D_DOWN:
		guide = Cell::Direction::DOWN;
		break;
	case (int)JoyStickPanel::D_LEFT:
		guide = Cell::Direction::LEFT;
		break;
	case (int)JoyStickPanel::D_RIGHT:
		guide = Cell::Direction::RIGHT;
		break;
	default:
		undo = true;
		break;
...
void Detect::onHit(cocos2d::Ref *ref, int touchType)
{
	if(_controlled == false) return;
	if (touchType == (int)ui::Widget::TouchEventType::BEGAN) 
	{
		if(_player != nullptr)
		{
			/*ValueMap model;
			model["typeId"] = 2008001;
			auto missile = dynamic_cast<Missile*>(CellFactory::getInstance()->createCell(model));
			addCell(missile, _player->getSite(),4, _player->getFloor()->getId(), _player->getFloor()->getTower()->getId());
			missile->fireToDirection(_player->getFace());*/
			_player->getFloor()->touchBeganUnit(_player, _player->getFaceSite());
		}
	}
	else if (touchType == (int)ui::Widget::TouchEventType::ENDED) 
	{
		...
		if(_player != nullptr)
		{
			_player->setGrabCell(nullptr);
			/*ValueMap model;
			model["typeId"] = 2008001;
			auto missile = dynamic_cast<Missile*>(CellFactory::getInstance()->createCell(model));
			addCell(missile, _player->getSite(),4, _player->getFloor()->getId(), _player->getFloor()->getTower()->getId());
			missile->fireToDirection(_player->getFace());*/
			_player->getFloor()->touchEndedUnit(_player, _player->getFaceSite());
			//_player->fall();

			getCellByName("switch_blue_up_1")->runPath("path_1");
		}
	}
}

void Detect::onJump(cocos2d::Ref *ref, int touchType)
{
	if (touchType == (int)ui::Widget::TouchEventType::ENDED) 
	{
		if(_controlled == false) return;
		if(_player != nullptr)
		{
			_player->jumpTo(_player->getSite());
		}
	}
}
...
bool Detect::onTouchBegan(Touch *touch, Event *unused_event)
{
	...
	auto dis = Vec2(winSize.width / 2, winSize.height / 2) * (getCurFloor()->getMapScale() - 1);
	touchLocation -= getCurFloor()->getPosition() - dis;
	touchLocation = touchLocation / getCurFloor()->getMapScale();
	auto site = POSTOSITE(touchLocation);
	if(_player != nullptr)
	{
		_player->moveToSite(site);
	}
	return false;
}
...
bool Detect::onTrigger(EventCustom *event)
{
	...
	else if((*data)["type"].asString() == "selectTrial")
	{
		auto typeId = effect["typeId"].asInt();
		auto trialsPanel = TrialsPanel::create("trials");
		trialsPanel->initWithTrials(getWorld()->getTrials(), typeId);
		PopPanel::getInstance()->addPanel(trialsPanel, 1);
	}
	return false;
}
