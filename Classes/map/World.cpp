...
void World::initModel()
{
	...
	// 关卡
	for(auto pair:Model::shareModel()->trialCsv->model)
	{
		auto id = pair.first;
		auto trial = Trial::create(id);
		_trials.insert(id,trial);
	}
...
{
	// 关卡
	auto trialsModel = map["trials"].asValueMap();
	for(auto pair:trialsModel)
	{
		auto id = cocos2d::Value(pair.first).asInt();
		auto trial = _trials.at(id);
		trial->loadModel(pair.second.asValueMap());
	}
	// 地图
...
ValueMap World::saveModel()
{
	...
	// 关卡
	ValueMap trials;
	for(auto pair : _trials)
	{
		trials[cocos2d::Value(pair.first).asString()] = pair.second->saveModel();
	}
	model["trials"] = trials;
...
Trial* World::getTrial(int trialId)
{
	auto trial = _trials.at(trialId);
	return trial;
}

Map<int, Trial*> World::getTrials()
{
	return _trials;
}
