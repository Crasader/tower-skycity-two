...
{
	// 虚拟键
	auto ppJoyLayOut = static_cast<Layout*>(Helper::seekWidgetByName(_root, "Panel_ppJoy"));
	_ppJoyPanel = PpJoyPanel::create("ppJoy");
	ppJoyLayOut->addChild(_ppJoyPanel);
	Detect::shareDetect()->setPpJoy(_ppJoyPanel);
...
void HudPanel::onButtonClicked(cocos2d::Ref *ref, Widget::TouchEventType touchType)
{
	...
	if (touchType == Widget::TouchEventType::ENDED) 
	{
		...
		case ACHIEVE:
			{
				if(_popPanel->hasPanelByName("trialCheck") != nullptr)
				{
					_popPanel->removePanelByName("trialCheck");
				}
				else
				{
					_popPanel->removeAllPanelsByOrder(1);
					auto trialCheck = TrialCheckPanel::create("trialCheck");
					trialCheck->initWithTrial(Detect::shareDetect()->getCurTrial(), false);
					_popPanel->addPanel(trialCheck,1);
				}
			}
			break;
		}
