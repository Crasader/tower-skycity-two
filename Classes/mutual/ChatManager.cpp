...
void ChatManager::setChat(int id)
{
	...
	if(actorName != "")
	{
		_actor = static_cast<Actor*>(Detect::shareDetect()->getCellByName(actorName));
		if(_actor == nullptr) _actor = static_cast<Actor*>(Detect::shareDetect()->getCellFromWaitingRoomByName(actorName));
		if(_actor != nullptr)
