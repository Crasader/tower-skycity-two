...
Model::Model()
{
	if(DATA_MODE == 1)
	{
		...
		getJsonFromFile("data/attr.json", attrDatas);
		getJsonFromFile("data/trial.json", trialDatas);
		getJsonFromFile("data/achievement.json", achievementDatas);
	}
	else if(DATA_MODE == 2)
	{
		...
		attrCsv = new CSVParse();
		attrCsv->openFile("data/attr.txt");
		trialCsv = new CSVParse();
		trialCsv->openFile("data/trial.txt");
		achievementCsv = new CSVParse();
		achievementCsv->openFile("data/achievement.txt");
	}
}
...
Model::~Model()
{
	...
	delete(achievementCsv);
	achievementCsv = nullptr;
}
